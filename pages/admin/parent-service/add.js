...
const AddParentServicePage = () => {
  const [fileImageMainList, setFileImageMainList] = useState([]);
  const [previewImage, setPreviewImage] = useState('');
  const [previewOpen, setPreviewOpen] = useState(false);
  const [previewTitle, setPreviewTitle] = useState('');
  const {mutate} = useSWR('list-service-parent-with-out-pagination')
      ...
      <div>
        ...
        <Form layout="vertical" autoComplete="on" onFinish={handleAddParentService}>
          ...
          <div className="field d-flex justify-content-center mt-2" style={{display :"flex"}}>
            <Button label="Add Service" icon="pi pi-check" style={{width: "auto"}} severity="sucess"
                    htmlType="submit"/>
          </div>
...
export default AddParentServicePage